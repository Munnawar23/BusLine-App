...
const Search = () => {
  ...
  const handleSearchBuses = () => {
    if (!from || !to) {
      Alert.alert(
        'Missing information',
        'Please select both deprature and destination locations.',
      );
      return;
    }
    if (from === to) {
      Alert.alert(
        'Invalid selection',
        'Deprature and destination location cannot be same.',
      );
      return;
    }
    ...
    if (date < today) {
      Alert.alert(
        'Invalid Date',
        'Please select a future day for your journey.',
      );
      return;
    }
    // ✅ FIX: Convert Date object to string before navigation
          ...
          <View className="my-4 border border-1 z-20 bg-white rounded-md border-gray-600">
            ...
            <View className="flex-row p-2 items-center justify-between">
              <View className="flex-row items-center">
                ...
                >
                  <Text className="font-bold text-base font-okra">Today</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  className="p-2 mr-2 rounded-lg bg-secondary"
                  onPress={() => {
                ...
                >
                  <Text className="font-bold text-base font-okra">
                    Tomorrow
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              >
                <View className="mr-3">
                  <Text className="text-base font-okra font-normal text-gray-500">
                    Date of Journey
                  </Text>
